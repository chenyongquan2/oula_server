        ...
        && outputBuffer_.readableBytes() == 0)
    {
        ...
        else if(nWrote < 0)
        {
            if(errno != EWOULDBLOCK)
            {
                ...
                {
                    //当对方已经关闭了连接conn，仍然往对方write数据，errno会返回EPIPE.
                    ...
                    errorOccurs = true;
                    //Todo:fix me, is this thread safe?
                    Logger::GetInstance()->debug("TcpConnection::send fail bcz peer closed, errno:{}", std::strerror(errno));
                }
            }
        }
    }
